...
// };
use risc0_zkvm::{
    default_executor, ExecutorEnv,
};
use guests::{
    SUBBLOCK_ETH_ELF, //SUBBLOCK_ETH_ID,
    AGG_ETH_ELF//, AGG_ETH_ID
};
...
async fn schedule_subblock_execution(
    ...
    for i in 0..inputs.subblock_inputs.len() {
        ...
        if execute {
            ...
            let exec_env = ExecutorEnv::builder()
                .write(&input).unwrap()
                .write(&parent_state).unwrap()
                .segment_limit_po2(21)
                .build().unwrap();
            let subblock_session_info = default_executor().execute(exec_env, SUBBLOCK_ETH_ELF).unwrap();
            // let subblock_instruction_count = report.total_instruction_count();
            let subblock_instruction_count = subblock_session_info.segments
                .iter()
                .fold(0, |acc, seg_info| acc + seg_info.cycles);
    ...
    if execute {
        assert_eq!(
            inputs.subblock_inputs.len(),
            inputs.subblock_outputs.len()
        );
        ...
        let exec_env = ExecutorEnv::builder()
            .write(&public_values).unwrap()
            // .write(&parent_state).unwrap()
            .write(&inputs.agg_input).unwrap()
            .write(&inputs.agg_input.parent_header().state_root).unwrap()
            .segment_limit_po2(21)
            .build().unwrap();
        let agg_session_info = default_executor().execute(exec_env, AGG_ETH_ELF).unwrap();
        ...
        let agg_instruction_count = agg_session_info.segments
            .iter()
            .fold(0, |acc, seg_info| acc + seg_info.cycles);
